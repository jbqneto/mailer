...
export interface RestRouteDependencies { projects: ProjectConfig[]; sendEmail: SendEmailUseCase; adminAuth: AdminAuth; adminLoginRateLimiter: AdminLoginRateLimiter; secureAdminCookie: boolean; deliveryStore: EmailDeliveryStore; rateLimiter: RateLimiter; metrics: GatewayMetrics; emailQueue?: EmailJobQueue | undefined; }
...
export function registerRestRoutes(app: FastifyInstance, dependencies: RestRouteDependencies): void {
  ...
  app.post('/admin/logout', async (request, reply) => { adminAuth.revoke(request.headers.cookie); return reply.code(204).header('Set-Cookie', clearAdminSessionCookie).send(); });
