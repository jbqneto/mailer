...
interface PreviewPageTemplate extends TemplatePreviewDefinition {
  name: string;
}

export function previewPage(
  templates: readonly PreviewPageTemplate[],
): string {
  ...
  const templateDefaults = Object.fromEntries(
    templates.map((template) => [template.name, template.data]),
  );
  const serializedDefaults = JSON.stringify(templateDefaults).replace(
    /</g,
    '\\u003c',
  );
...
<html lang="pt-BR">
  ...
  <body>
    <header>
      <div>
        <h1>Email Gateway · Template Preview</h1>
        <p>Renderize um template e envie um teste pelo mesmo gateway, sem servidor adicional.</p>
      </div>
    ...
    <main>
      <section class="panel">
        <div class="field">
          <label for="apiKey">API key do projeto</label>
          <input id="apiKey" type="password" autocomplete="off" placeholder="Bearer token" />
          <div id="projectStatus" class="hint">Informe uma API key para identificar o projeto.</div>
        </div>
        <div class="field">
  ...
  "actionUrl": "https://example.com/activate"
        ...
        <button id="render" type="button">Atualizar preview</button>
        <p id="status" class="hint">O preview não envia e-mail.</p>
      </section>
      <section class="preview">
        <div class="preview-head">
          <strong>Resultado renderizado</strong>
          <span id="subject">Sem preview</span>
        </div>
      ...
      <section class="recent">
        <div class="recent-head">
          ...
          <button id="refreshRecent" type="button">Atualizar</button>
        </div>
        <table>
          <thead><tr><th>ID</th><th>Template</th><th>Destinatário</th><th>Status</th><th>Data</th></tr></thead>
          <tbody id="recentRows"><tr><td class="empty" colspan="5">Nenhum envio registrado.</td></tr></tbody>
        </table>
      </section>
    </main>
    <form id="sendBar" class="send-bar">
      <span>Enviar e-mail de teste</span>
      <input id="recipient" type="email" required placeholder="destinatario@example.com" aria-label="Destinatário" />
      <button id="send" type="submit">Enviar teste</button>
    </form>
    <script>
      ...
      const templateDefaults = ${serializedDefaults};
      ...
      function readData() {
        try {
          ...
          throw new Error('Os dados precisam ser um JSON válido.');
        }
      }
      ...
      async function errorMessage(response) {
        try {
          ...
          return body.message || body.error || 'Pedido rejeitado (' + response.status + ').';
        } catch {
          return 'Pedido rejeitado (' + response.status + ').';
        }
      }

      async function identifyProject() {
        if (!apiKey.value.trim()) {
          projectStatus.textContent = 'Informe uma API key para identificar o projeto.';
          projectStatus.className = 'hint';
          return;
        }

        const response = await fetch('/v1/projects/me', {
          headers: { 'Authorization': 'Bearer ' + apiKey.value.trim() }
        });
        if (!response.ok) {
          projectStatus.textContent = 'API key inválida ou projeto não encontrado.';
          projectStatus.className = 'error';
          return;
        }
        const project = await response.json();
        projectStatus.textContent = project.projectId + ' · from ' + project.fromEmail;
        projectStatus.className = 'success';
      }

      async function renderPreview() {
        setStatus('A renderizar...', 'hint');
        render.disabled = true;
        try {
          const response = await fetch('/v1/emails/preview', requestOptions({
            template: template.value,
            data: readData()
          }));
          if (!response.ok) throw new Error(await errorMessage(response));
          frame.srcdoc = await response.text();
          subject.textContent = response.headers.get('X-Email-Subject') || '';
          setStatus('Preview atualizado. Nenhum e-mail foi enviado.', 'success');
        } catch (error) {
      ...
      async function loadRecentSends() {
        const response = await fetch('/admin/emails?limit=20');
        if (!response.ok) return;
        const result = await response.json();
        recentRows.replaceChildren();
        if (!result.data.length) {
          const row = document.createElement('tr');
          row.innerHTML = '<td class="empty" colspan="5">Nenhum envio registrado.</td>';
          recentRows.appendChild(row);
          return;
        }
        for (const delivery of result.data) {
          const row = document.createElement('tr');
          const values = [delivery.id, delivery.template, delivery.to.join(', '), delivery.status, new Date(delivery.createdAt).toLocaleString()];
      ...
      refreshRecent.addEventListener('click', () => { void loadRecentSends(); });
      apiKey.addEventListener('change', () => { void identifyProject(); });
      logout.addEventListener('click', async () => {
        await fetch('/admin/logout', { method: 'POST' });
        window.location.href = '/preview';
      });
      template.addEventListener('change', () => {
        data.value = JSON.stringify(templateDefaults[template.value], null, 2);
      });

      sendBar.addEventListener('submit', async (event) => {
        event.preventDefault();
        setStatus('A enviar...', 'hint');
        send.disabled = true;
        try {
          const response = await fetch('/v1/emails', requestOptions({
            template: template.value,
            to: document.getElementById('recipient').value.trim(),
            data: readData()
          }));
          if (!response.ok) throw new Error(await errorMessage(response));
          const result = await response.json();
          setStatus('E-mail aceite. ID: ' + result.id + ' · Message ID: ' + result.messageId, 'success');
