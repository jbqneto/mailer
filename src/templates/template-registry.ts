...
import GenericWelcomeEmail from '../../templates/shared/welcome-email.js';
import GenericEmailConfirmationEmail from '../../templates/shared/email-confirmation.js';
import GenericPasswordResetEmail from '../../templates/shared/password-reset.js';
...
function defineTemplate<T extends object>(definition: TemplateDefinition<T>): TemplateDefinition<T> {
  return definition;
}
...
const genericWelcomeSchema = z.object({
  brand: z.string().trim().min(1).max(120),
  logoUrl: z.string().url().optional(),
  headerBackgroundColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
  footerBackgroundColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
  ...
  containerBackgroundColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
  title: z.string().trim().min(1).max(180),
  mainText: z.string().trim().min(1).max(10_000),
...
const genericEmailConfirmationSchema = z.object({
  brand: z.string().trim().min(1).max(120),
  logoUrl: z.string().url().optional(),
  headerBackgroundColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
  footerBackgroundColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
  ...
  expiresIn: z.string().trim().min(1).max(60).optional(),
});

const genericPasswordResetSchema = z.object({
  brand: z.string().trim().min(1).max(120),
  logoUrl: z.string().url().optional(),
  headerBackgroundColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
  footerBackgroundColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
  ...
  unsubscribeUrl: z.string().url().optional(),
  expiresIn: z.string().trim().min(1).max(60).optional(),
  requestedIp: z.string().trim().min(1).max(60).optional(),
  requestedLocation: z.string().trim().min(1).max(120).optional(),
});
...
export const templateRegistry = {
  ...
  'generic-welcome': defineTemplate({
    schema: genericWelcomeSchema,
    component: GenericWelcomeEmail,
    subject: (data) => data.title,
  }),
  'generic-email-confirmation': defineTemplate({
    schema: genericEmailConfirmationSchema,
    component: GenericEmailConfirmationEmail,
    subject: (data) => data.title,
  }),
  'generic-password-reset': defineTemplate({
    schema: genericPasswordResetSchema,
    component: GenericPasswordResetEmail,
    subject: (data) => data.title,
  }),
};
